#pragma once

#include <fmt/format.h>

// Taken from https://github.com/travisvroman/kohi/blob/main/kohi.core/src/debug/kassert.h
// Try via __has_builtin first.
#if defined(__has_builtin) && !defined(__ibmxl__)
#    if __has_builtin(__builtin_debugtrap)
#        define ENVY_DEBUG_BREAK() __builtin_debugtrap()
#    elif __has_builtin(__debugbreak)
#        define ENVY_DEBUG_BREAK() __debugbreak()
#    endif
#endif

// If not setup, try the old way.
#if !defined(ENVY_DEBUG_BREAK)
#    if defined(__clang__) || defined(__gcc__)
#        define ENVY_DEBUG_BREAK() __builtin_trap()
#    elif defined(_MSC_VER)
#        include <intrin.h>
#        define ENVY_DEBUG_BREAK() __debugbreak()
#    else
// Fall back to x86/x86_64
#        define ENVY_DEBUG_BREAK() asm { int 3 }
#    endif
#endif

#ifndef ENVY_RELEASE
#   define ENVY_ASSERT(expr, msg)                                                        \
        {                                                                                \
            if (expr) {}                                                                 \
            else {                                                                       \
                fmt::println("[ASSERTION FAILED | {}:{}] {}", __FILE__, __LINE__, msg);  \
                ENVY_DEBUG_BREAK();                                                      \
            }                                                                            \
        }                                   
#else
#   define ENVY_ASSERT(expr, msg)
#endif